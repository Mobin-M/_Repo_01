...
# this function returns a complex sinusoid of time length len (in second), not necessarily periodic in discrete-time
...
def Discrete_Complex_Sinusoid_1(A, phi, f0, fs, t_len, request_periodic=0):
    ...
    if(request_periodic == 0):
        ...
        x_n_array = np.zeros((len(fd), N), dtype=np.complex128)     # initializing an 2D array of dimension len(fd) x N: x_n has len(fd) elements in which each of them has N elements
        
        # x_t = A*exp(1j*2*pi*f0*t + phi)   ------>   x_n = A*exp(2*pi*fd*n + phi)
        for x in range(len(fd)):            #
            x_n_array[x] = A[x]*np.exp(1j*2*np.pi*fd[x]*n + phi[x])     #x_n_array is each constucting sinusoid in discrete time before summing up all together
            
        # Summing up all the constructing sinusoids
        x_n = np.zeros((1, N), dtype=np.complex128)
        for x in range(len(fd)):
            x_n = x_n + x_n_array[x]
        x_n = x_n[0,:]  
    ...
    ## requested to return periodic DT sinusoid
        ...
        # x_t = A*exp(1j*2*pi*f0*t + phi)   ------>   x_n = A*exp(2*pi*fd*n + phi)
        for x in range(len(fd)):            #
            x_n_array[x] = A[x]*np.exp(1j*2*np.pi*fd[x]*n + phi[x])
...
# this function returns a complex sinusoid of time length len (in second), not necessarily periodic in discrete-time
# the input parameters are number of samples, N, and number of rotation in full length (m). 
# Note that in case N and m are both integers, it means that the returned signal is a full period in discrete-time
# (for now) it is considered that discrete-time sinusoid constructed this way only are one sinusoid
def Discrete_Complex_Sinusoid_2(N, m, A, phi):
    print('Creating complex sinusoid')
    
    x_n = np.zeros((1, N), dtype=np.complex128)
    n = np.arange(0,N)
    x_n = A*np.exp(1j*2*np.pi*(m/N)*n + phi)    # constructed single component discrete time sinusoid
                                                # note that if N and m are both integers means that in N samples the sinusoid will have m rotation and reaching full period

    return(x_n)


# Implementation like the complex counter part "Discrete_Complex_Sinusoid_1", with a minor change that instead
# of using exponentionals np.exp(1j*2np.pi*..) is used a cosine np.cos(2*np.pi*.....)
...
# Implementation like the complex counter part "Discrete_Complex_Sinusoid_2", with a minor change that instead
# of using exponentionals np.exp(1j*2np.pi*..) is used a cosine np.cos(2*np.pi*.....)
def Discrete_Real_Sinusoid_2(N, m, A, phi):
    print('Creating Real sinusoid')

    x_n = np.zeros((1, N))
    n = np.arange(0,N)
    x_n = A*np.cos(2*np.pi*(m/N)*n + phi)       # constructed single component discrete time sinusoid
                                                # note that if N and m are both integers means that in N samples the sinusoid will have m rotation and reaching full period
